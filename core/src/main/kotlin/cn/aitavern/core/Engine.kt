...
object Engine {
    ...
    fun prompt(character: Character, room: ChatRoom, members: List<Character>, history: List<Message>, entries: List<LoreEntry>): List<WireMessage> {
        val system = buildString {
            ...
            section("用户人设", room.persona); section("房间场景", room.scenario)
        ...
        val output = mutableListOf(WireMessage("system", substitute(system, character.name, room.userName)))
        history.filter { it.sequence > room.summaryThrough && it.status == "complete" }.forEach { msg ->
            ...
            output += WireMessage(if (msg.speakerId == null) "user" else "assistant", "[$name] ${msg.text}")
        }
    ...
    fun branch(room: ChatRoom, history: List<Message>, index: Int, replacement: String? = null): Pair<ChatRoom,List<Message>> {
        ...
        val branch = room.copy(id=newId(), name="${room.name} · 分支", parentId=room.id, summary="", longSummary="", summaryThrough=-1, createdAt=System.currentTimeMillis())
