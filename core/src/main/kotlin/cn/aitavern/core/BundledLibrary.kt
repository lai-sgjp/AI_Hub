...
object BundledLibrary {
    fun load(manifest: BundleManifest,read: (String)->ByteArray): BundleImport {
        ...
        val assetPrefix=manifest.assetPrefix.trim('/')
        require(assetPrefix.isEmpty() || (':' !in assetPrefix && '\\' !in assetPrefix && assetPrefix.split('/').none { it==".." || it=="." || it.isBlank() })) { "内置资源前缀无效" }
        fun assetPath(path: String)=if(assetPrefix.isBlank()) path else "$assetPrefix/$path"
        fun input(path: String): ByteArray {
            ...
            return read(assetPath(path)).also { require(it.size<=CardCodec.MAX_FILE) { "内置资源过大" } }
        }
        fun gallery(path: String, images: List<BundleImage>): List<CharacterImage> = images.mapIndexed { index,image ->
            ...
            input(image.path)
            CharacterImage(id=stable("$path/gallery/$index"),title=image.title.ifBlank { "立绘 ${index+1}" },assetPath=assetPath(image.path),mimeType=image.mimeType)
        }
